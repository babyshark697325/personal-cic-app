...
interface CalendarWidgetDashboardProps {
  monthNames: string[];
  selectedMonth: number;
  setSelectedMonth: (idx: number) => void;
  monthDropdownOpen: boolean;
  setMonthDropdownOpen: (open: boolean) => void;
}

export default function CalendarWidgetDashboard({
  monthNames,
  selectedMonth,
  setSelectedMonth,
  monthDropdownOpen,
  setMonthDropdownOpen,
}: CalendarWidgetDashboardProps) {
  ...
  const computeWeekDaysForMonth = () => {
    ...
    return Array.from({ length: 7 }, (_, i) => {
      ...
      return {
        ...
        isToday: d.getDate() === currentDay && d.getMonth() === currentMonthIndex && d.getFullYear() === currentYear,
      };
    });
  };
  ...
  const selectedDateEvents = events.filter(event => event.date === selectedDate && (event.month === selectedMonth || event.month === undefined));
    ...
    <div>
      ...
      <div className="grid grid-cols-7 gap-1 w-full justify-items-center mt-1">
        {(() => {
          ...
          const prevMonthDays = new Date(prevMonthYear, month, 0).getDate();
          for (let i = 0; i < firstDay; i++) {
            const dayNum = prevMonthDays - (firstDay - 1) + i;
            const dayStr = dayNum.toString().padStart(2, '0');
            const hasEvent = events.some(event => event.date === dayStr && (event.month === prevMonth || event.month === undefined));
            cells.push(
              <button
                key={`prev-${dayStr}-${i}`}
                onClick={() => {
                  // navigate to previous month and select that date
                  const newMonth = prevMonth;
                  const newYear = prevMonth === 11 ? year - 1 : year;
                  setSelectedMonth(newMonth);
                  setSelectedYear(newYear);
                  setSelectedDate(dayStr);
                }}
                className={`h-8 w-8 text-sm rounded-full transition-all font-medium border-0 relative flex items-center justify-center text-gray-300`}
              >
                {dayStr}
                {hasEvent && (
          ...
          for (let date = 1; date <= daysInMonth; date++) {
            ...
            const hasEvent = events.some(event => event.date === dayStr && (event.month === month || event.month === undefined));
          ...
          // Trailing days from next month (faded) to fill the grid
          ...
          while (cells.length % 7 !== 0) {
            ...
            const hasEvent = events.some(event => event.date === dayStr && (event.month === nextMonth || event.month === undefined));
            cells.push(
              <button
                key={`next-${dayStr}-${cells.length}`}
                onClick={() => {
